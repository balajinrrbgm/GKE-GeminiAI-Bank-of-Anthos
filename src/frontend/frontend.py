...
def create_app():
    ...
    @app.route('/api/ai/insights/<username>', methods=['GET'])
    def get_ai_insights(username):
        """
        Get AI-powered financial insights for a user
        """
        # For testing, temporarily disable authentication
        # In production, enable the authentication check below
        # auth_token = request.cookies.get(app.config['TOKEN_NAME'])
        # if not verify_token(auth_token):
        #     return jsonify({"error": "Unauthorized"}), 401
        
        try:
            # Forward request to AI assistant service
            ai_response = requests.get(
                f'http://{app.config["AI_ASSISTANT_URI"]}/insights/{username}',
                timeout=10
            )
            ...
            if ai_response.status_code == 200:
                ...
                app.logger.error(f'AI insights error: {ai_response.status_code}')
                return jsonify({"error": "AI service unavailable"}), 503
                
        except Exception as e:
            app.logger.error(f'AI insights request failed: {str(e)}')
            return jsonify({"error": "AI service unavailable"}), 503

    @app.route('/api/ai/chat', methods=['POST'])
    def ai_chat():
        """
        Handle chat requests to AI assistant
        """
        # For testing, temporarily disable authentication
        # In production, enable the authentication check below
        # auth_token = request.cookies.get(app.config['TOKEN_NAME'])
        ...
        try:
            ...
            ai_response = requests.post(
                f'http://{app.config["AI_ASSISTANT_URI"]}/chat',
                json=request.json,
                timeout=10
            )
            ...
            app.logger.error(f'AI chat request failed: {str(e)}')
            return jsonify({"error": "AI service unavailable"}), 503

    @app.route('/api/ai/spending-analysis/<username>', methods=['GET'])
    def get_spending_analysis(username):
        """
        Get spending analysis from AI assistant
        """
        # For testing, temporarily disable authentication
        # In production, enable the authentication check below
        # auth_token = request.cookies.get(app.config['TOKEN_NAME'])
        ...
        try:
            # Forward request to AI assistant service
            ai_response = requests.get(
                f'http://{app.config["AI_ASSISTANT_URI"]}/spending-analysis/{username}',
                timeout=10
            )
            ...
            app.logger.error(f'AI spending analysis request failed: {str(e)}')
            return jsonify({"error": "AI service unavailable"}), 503
    ...
    app.config["AI_ASSISTANT_URI"] = os.environ.get('AI_ASSISTANT_API_ADDR', 'ai-assistant:8080')
